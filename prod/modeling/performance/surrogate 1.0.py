...
def create_model(data_path, primary_dim, secondary_dim, lattent_dim, model_path, epochs=50, learning_rate=0.001):
    ...
    for sp in split:
        ...
        model = ann.Surrogate(primary_dim, secondary_dim, lattent_dim, Y.shape[1],
        ...
        trainer.train(tr[0], (tr[1] - Y_MIDDLE) / Y_DISPERSION, val[0], (val[1] - Y_MIDDLE) / Y_DISPERSION,
                      epochs=epochs, verbose=0, batch_size=50, model_path=model_path)
...
for index, d in enumerate(data):
    ...
    results['Surrogate'][d] = {'AESE': np.average(out['stop_epoch']),
